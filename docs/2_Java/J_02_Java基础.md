...
`==` 用于比较两个变量**是否指向同一个内存地址**（即是否是同一个对象）。

- **基本数据类型**：`==` 比较的是值是否相等。
- **引用数据类型**（对象）：`==` 比较的是**两个对象的引用地址**是否相同，而不是对象的内容是否相同。

`equals()` 是 `Object` 类中的方法，默认行为与 `==` 一样（即比较内存地址）。

**许多类（如 `String`、`Integer` 等）都重写了 `equals()`，使其比较的是内容，而不是地址。**

**如果需要自定义对象的内容比较，一般需要重写 `equals()` 方法。**

| 比较方式           | 适用类型     | 比较内容 | 是否可重写 |
| ------------------ | ------------ | -------- | ---------- |
| `==`               | 基本数据类型 | 值       | ❌          |
| `==`               | 引用数据类型 | 内存地址 | ❌          |
| `equals()`（默认） | 引用数据类型 | 内存地址 | ❌          |
| `equals()`（重写） | 引用数据类型 | 对象内容 | ✅          |
